...
# 独立 ROS domain，验证真实机械臂运动必须先确认底盘停稳或显式发布停车。
export ROS_DOMAIN_ID="${PIPER_BASE_STOP_GATE_ROS_DOMAIN_ID:-88}"

LOG_DIR="${WORKSPACE_DIR}/log"
mkdir -p "${LOG_DIR}"
CONTROL_LOG="${LOG_DIR}/piper_base_stop_gate_control_$(date +%Y%m%d_%H%M%S).log"
TASK_LOG="${LOG_DIR}/piper_base_stop_gate_task_$(date +%Y%m%d_%H%M%S).log"
CONTROL_PID=""
TASK_PID=""
...
echo "[Piper BaseStop Gate] 使用 ROS_DOMAIN_ID=${ROS_DOMAIN_ID}"
echo "[Piper BaseStop Gate] 启动控制桥，日志：${CONTROL_LOG}"
...
echo "[Piper BaseStop Gate] 启动已标定但未确认底盘停止的任务层，日志：${TASK_LOG}"
setsid ros2 launch slam_nav_piper_manipulation piper_manipulation.launch.py \
  ...
  hand_eye_calibrated:=true \
  hand_eye_result_must_exist:=false \
  >"${TASK_LOG}" 2>&1 &
TASK_PID="$!"
...
class PiperBaseStopGateSmoke(Node):
    """验证真实机械臂运动缺少底盘停止确认时必须安全拒绝。"""

    def __init__(self):
        super().__init__('piper_base_stop_gate_smoke_client')
        self.latest_state = {}
    ...
    def wait_control_ready(self):
        ...
        while rclpy.ok() and time.time() < deadline:
            ...
            if (
                ...
                self.get_logger().info(f'控制桥已处于真实运动可检查状态：{self.latest_state}')
                return True
    ...
    def run(self):
        ...
        goal.object_id = 'base_stop_gate_target'
        ...
        if result.success:
            ...
            return 2
        if '底盘停止' not in result.message and '导航暂停' not in result.message:
            self.get_logger().error(f'pick action 拒绝原因应包含底盘停止/导航暂停，实际：{result.message}')
            return 2

        self.get_logger().info(f'真实运动缺少底盘停止确认时按预期拒绝：{result.message}')
        return 0


def main():
    rclpy.init()
    node = PiperBaseStopGateSmoke()
    try:
        exit_code = node.run()
    finally:
        node.destroy_node()
...
if [[ "${SMOKE_STATUS}" -ne 0 ]]; then
  echo "[Piper BaseStop Gate] 烟测失败，控制桥日志最后 80 行：" >&2
  tail -n 80 "${CONTROL_LOG}" >&2 || true
  echo "[Piper BaseStop Gate] 任务层日志最后 120 行：" >&2
  tail -n 120 "${TASK_LOG}" >&2 || true
  exit "${SMOKE_STATUS}"
fi

echo "[Piper BaseStop Gate] 底盘停止真实运动门禁烟测通过。"
