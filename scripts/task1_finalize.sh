...
说明：
  ...
  默认流程：查看 task1 状态 -> 编译结课报告 PDF -> strict 预检 -> strict 交付检查 -> 预览压缩包。
...
step "3/5 运行 task1 结构预检"
run_check task1_preflight.sh "${preflight_args[@]}"

step "4/5 运行 task1 交付检查"
