...
# 真实机器人传感器启动辅助：用于需要 FAST-LIO 输出的入口。
# 只清理本脚本启动的进程；已经运行的驱动会被复用。
...
real_sensor_warn_host_ip() {
  local host_ip="${REAL_SENSOR_LIDAR_HOST_IP:-${LIDAR_HOST_IP:-192.168.1.50}}"
...
real_sensor_start_livox_driver() {
  ...
  setsid "${REAL_SENSOR_SCRIPT_DIR}/start_livox_mid360.sh" &
  REAL_SENSOR_LIVOX_LAST_PID="$!"
...
start_real_lidar_inputs() {
  ...
  if [[ "${start_livox_now}" == "true" ]]; then
    real_sensor_start_livox_driver
    real_sensor_wait_for_livox_pair "${lidar_topic}" "${raw_imu_topic}" false "${livox_timeout}" || true
  else
    real_sensor_wait_for_livox_pair "${lidar_topic}" "${raw_imu_topic}" false "${livox_timeout}" || true
  fi
  ...
  if [[ "${need_imu_filter}" == "true" ]]; then
    if [[ "${REAL_SENSOR_RAW_IMU_READY}" != "true" ]]; then
      echo "[real-sensors] WARN: raw IMU missing; starting IMU filter once so it can wait for late data" >&2
    fi
    real_sensor_start_imu_filter
  ...
  if (( failures > 0 )); then
    if [[ "${strict}" =~ ^(true|1|yes|on)$ ]]; then
      ...
      echo "[real-sensors]        请运行: scripts/livox_debug.sh" >&2
      return 1
    fi
    echo "[real-sensors] WARN: 实机传感器链路暂未完整就绪，缺失 ${failures} 个关键发布者；继续启动。" >&2
    echo "[real-sensors] WARN: 如果话题一直不出现，请运行: scripts/livox_debug.sh" >&2
  fi
}
