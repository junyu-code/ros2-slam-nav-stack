...
echo "[Piper Full] 1/14 安全配置检查..."
"${SCRIPT_DIR}/piper_safety_check.sh"

echo
echo "[Piper Full] 2/14 边界回归检查..."
"${SCRIPT_DIR}/piper_boundary_check.sh"

echo
echo "[Piper Full] 3/14 依赖预检和官方包检查..."
"${SCRIPT_DIR}/piper_preflight.sh" --require-official

echo
echo "[Piper Full] 4/14 官方 URDF -> 项目侧 piper_* frame 审计..."
ros2 run slam_nav_piper_bringup piper_official_frame_audit.py --check-project-adapter

echo
echo "[Piper Full] 5/14 项目侧 MoveIt2 配置映射审计..."
"${SCRIPT_DIR}/piper_moveit_config_audit.sh"

echo
echo "[Piper Full] 6/14 Piper 手眼标定配置边界检查..."
"${SCRIPT_DIR}/piper_hand_eye_check.sh"

echo
echo "[Piper Full] 7/14 Piper 运行时 TF 链烟测..."
"${SCRIPT_DIR}/piper_tf_smoke.sh"

echo
echo "[Piper Full] 8/14 Piper runtime 命名空间边界烟测..."
"${SCRIPT_DIR}/piper_namespace_smoke.sh"

echo
echo "[Piper Full] 9/14 Piper 控制桥安全边界烟测..."
"${SCRIPT_DIR}/piper_control_smoke.sh"

echo
echo "[Piper Full] 10/14 Piper 实机入口 dry-run 安全拒绝烟测..."
"${SCRIPT_DIR}/piper_real_dry_run.sh"

echo
echo "[Piper Full] 11/14 Headless Gazebo 官方 Piper 组合模型烟测..."
"${SCRIPT_DIR}/piper_gazebo_smoke.sh"

echo
echo "[Piper Full] 12/14 Fake RGB-D -> grasp candidates -> pick/place action 烟测..."
"${SCRIPT_DIR}/piper_task_smoke.sh"

echo
echo "[Piper Full] 13/14 Piper 学习层抓取候选排序烟测..."
"${SCRIPT_DIR}/piper_learning_smoke.sh"

echo
echo "[Piper Full] 14/14 MoveIt2 plan-only 烟测..."
