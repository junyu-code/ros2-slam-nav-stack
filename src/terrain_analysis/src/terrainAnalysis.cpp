...
std::string outputTerrainTopic = "/terrain_map";
...
int main(int argc, char **argv) {
  ...
  RCLCPP_INFO(nh->get_logger(), "terrain_analysis input cloud: %s", inputCloudTopic.c_str());
  RCLCPP_INFO(nh->get_logger(), "terrain_analysis odometry: %s", odometryTopic.c_str());
  RCLCPP_INFO(nh->get_logger(), "terrain_analysis output terrain: %s", outputTerrainTopic.c_str());
  ...
  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>(
      inputCloudTopic, 10, laserCloudHandler);
