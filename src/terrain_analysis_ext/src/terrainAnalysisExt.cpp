...
std::string inputCloudTopic = "/cloud_registered_body";
std::string localTerrainTopic = "/terrain_map";
std::string joyTopic = "/joy";
std::string clearingTopic = "/cloud_clearing";
std::string outputTerrainTopic = "/terrain_map_ext";
...
int main(int argc, char **argv) {
  ...
  nh->declare_parameter<std::string>("odometryTopic", odometryTopic);
  nh->declare_parameter<std::string>("inputCloudTopic", inputCloudTopic);
  nh->declare_parameter<std::string>("localTerrainTopic", localTerrainTopic);
  nh->declare_parameter<std::string>("joyTopic", joyTopic);
  nh->declare_parameter<std::string>("clearingTopic", clearingTopic);
  nh->declare_parameter<std::string>("outputTerrainTopic", outputTerrainTopic);
  ...
  nh->get_parameter("odometryTopic", odometryTopic);
  nh->get_parameter("inputCloudTopic", inputCloudTopic);
  nh->get_parameter("localTerrainTopic", localTerrainTopic);
  nh->get_parameter("joyTopic", joyTopic);
  nh->get_parameter("clearingTopic", clearingTopic);
  nh->get_parameter("outputTerrainTopic", outputTerrainTopic);

  RCLCPP_INFO(nh->get_logger(), "terrain_analysis_ext input cloud: %s", inputCloudTopic.c_str());
  RCLCPP_INFO(nh->get_logger(), "terrain_analysis_ext local terrain: %s", localTerrainTopic.c_str());
  RCLCPP_INFO(nh->get_logger(), "terrain_analysis_ext output terrain: %s", outputTerrainTopic.c_str());
  ...
  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>(
      inputCloudTopic, 5, laserCloudHandler);

  auto subJoystick =
      nh->create_subscription<sensor_msgs::msg::Joy>(joyTopic, 5, joystickHandler);

  auto subClearing = nh->create_subscription<std_msgs::msg::Float32>(
      clearingTopic, 5, clearingHandler);

  auto subTerrainCloudLocal =
      nh->create_subscription<sensor_msgs::msg::PointCloud2>(
          localTerrainTopic, 2, terrainCloudLocalHandler);
